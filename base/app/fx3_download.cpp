...
#include <QtCore>
#include <QtGui>
#include <QtWidgets>       // covers all widgets below
...
static int ram_write(unsigned char *buf, unsigned int ramAddress, int len)
{
    ...
    printf("Starting RAM write: %d bytes to address 0x%08X\n", len, ramAddress);

    while (len > 0) {
        size = (len > MAX_WRITE_SIZE) ? MAX_WRITE_SIZE : len;

        printf("Writing %d bytes to RAM address 0x%08X (buffer offset %d)\n",
               size, ramAddress, index);

        // Optional: print the first few bytes of this chunk
        printf("Data chunk: ");
        for (int i = 0; i < size && i < 16; i++) { // only first 16 bytes for readability
        ...
        if (r != size) {
            ...
            printf("Requested: %d bytes, Written: %d bytes\n", size, r);
            printf("Failed at RAM address 0x%08X, buffer offset %d\n", ramAddress, index);
            return -1;
        }
    ...
    printf("RAM write completed successfully\n");
    return 0;
}
