...
static char le3_out_data[4096] = {0};
...
void ControlCenter::on_streamer_control_start_clicked ()
{
	...
	temp = mainwin->streamer_size_sel->currentText().toStdString().c_str();
	sscanf (temp, "%d", &reqsize);
	temp = mainwin->streamer_queue_sel->currentText().toStdString().c_str();
...
static void update_endpoints()
{
	...
	mainwin->streamer_ep_sel->clear ();
	mainwin->streamer_size_sel->clear ();
	mainwin->streamer_queue_sel->clear ();
	mainwin->streamer_out_passcnt->setText ("0");
	mainwin->streamer_out_failcnt->setText ("0");
	mainwin->streamer_out_perf->setText ("0");
	mainwin->streamer_control_start->setEnabled (false);
	...
	for ( i = 0; i < summ_count; ++i ) {
		...
		mainwin->streamer_ep_sel->addItem(tbuf);
		epfound = true;
	}
	...
	if (epfound) {
		...
		for (int size = 1; size <= 512; size *= 2) {
			sprintf (tbuf, "%d", size);
			mainwin->streamer_size_sel->addItem (tbuf);
			mainwin->streamer_queue_sel->addItem (tbuf);
		}
...
void ControlCenter::on_le3_out_ascii_textChanged()
{
	...
	QByteArray t = mainwin->le3_out_ascii->text().toUtf8();
...
void ControlCenter::on_le3_out_hex_textEdited()
{
	...
	QByteArray t = mainwin->le3_out_hex->text().toUtf8();
...
static void dump_data(unsigned short num_bytes, char *dbuf)
{
...
void ControlCenter::on_le6_out_hex_textEdited()
{
	...
	QByteArray t = mainwin->le6_out_hex->text().toUtf8();
	QByteArray t2 = QByteArray::fromHex(t);

	sz = (t2.size () > 4096) ? 4096 : t2.size();
	memcpy (le6_out_data, (const char *)t2.data(), sz);

	mainwin->le6_out_ascii->setText("");
...
void ControlCenter::on_le6_out_ascii_textEdited()
{
	...
	QByteArray t = mainwin->le6_out_ascii->text().toUtf8();

	sz = (t.size () > 4096) ? 4096 : t.size();
	memcpy (le6_out_data, (const char *)t.data(), sz);

	mainwin->le6_out_hex->setText("");
	mainwin->le6_outfile->setText("");
...
void ControlCenter::pb6_send_nofile_selected()
{
	...
	if ( (mainwin->le6_out_hex->text() == "" ) && (mainwin->le6_out_ascii->text() == "") ) {
